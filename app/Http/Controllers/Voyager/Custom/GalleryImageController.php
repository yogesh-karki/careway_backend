...
class GalleryImageController extends VoyagerBaseController
{
    ...
    {
        $request['subtitle'] = $request->subtitle == null ? "[]" : json_encode($request->subtitle);
        $slug = $this->getSlug($request);
    ...
    public function store(Request $request)
    {
        if($request->images)
        {
        $request['subtitle'] = json_encode(array_fill(0, count($request->images), null));
        }
    ...
    public function deleteBreadImages($data, $rows, $single_image = null)
    {
        ...
        foreach ($rows as $row) {
            ...
            foreach ($images_to_remove as $key=>$image) {
                ...
                if ($image != config('voyager.user.default_avatar') && (is_null($single_image) || $single_image == $image)) {
                    if($row->type == 'multiple_images')
                    {
                        $sub = json_decode($data->subtitle);
                        array_splice($sub, $key, 1);
                        $data->subtitle = json_encode($sub);
                        $data->update();                        
                    }
    ...
    public function order(Request $request)
    {
        ...
        $results = $model->orderBy($dataType->order_column, $dataType->order_direction)->get()->except(1);
		
		if($request->id)
		{
			$imgarr = [];
			$i  = 0;
			$album = json_decode($model->orderBy($dataType->order_column, $dataType->order_direction)->find($request->id)->images);
			$subtitle = json_decode($model->orderBy($dataType->order_column, $dataType->order_direction)->find($request->id)->subtitle);
			if(count($album)>0){		
			...
			$results =  $imgarr;
			return view('includes.multiple_image_order',compact(
				'results'
			));
		}
		...
		if($request->id){
			...
			$subtitle = [];
				foreach(json_decode($request->order) as $item){
					array_push($album,$item->image);
					array_push($subtitle,$item->caption);
				}
			$gallery = \App\Gallery::find($request->id);
			$gallery->images = json_encode($album);
			$gallery->subtitle = json_encode($subtitle);
			$gallery->save();
			return 'done';
